...
use crate::preview2::wasi::rdbms::postgres::{
    ...
    EnumerationType, Error, Host, HostDbConnection, HostDbResultSet, HostLazyDbColumnType,
    HostLazyDbValue, Int4bound, Int4range, Int8bound, Int8range, Interval, IpAddress, Numbound,
    Numrange, Time, Timestamp, Timestamptz, Timetz, Tsbound, Tsrange, Tstzbound, Tstzrange,
};
...
impl<Ctx: WorkerCtx> HostDbConnection for DurableWorkerCtx<Ctx> {
    ...
    async fn query(
        ...
        match postgres_utils::to_db_values(params, self.as_wasi_view().table()) {
            Ok(params) => {
                let result = self
                    .state
                    .rdbms_service
...
impl<Ctx: WorkerCtx> HostDbResultSet for DurableWorkerCtx<Ctx> {
    async fn get_columns(
        ...
        let columns = postgres_utils::from_db_columns(columns, self.as_wasi_view().table())
    ...
    async fn get_next(
        ...
        let rows = match rows {
            Some(rows) => {
                let result = postgres_utils::from_db_rows(rows, self.as_wasi_view().table())
                    .map_err(Error::QueryResponseFailure)?;
                Some(result)
            }
            None => None,
...
impl<Ctx: WorkerCtx> HostLazyDbColumnType for DurableWorkerCtx<Ctx> {
    async fn create(
        ...
        let value = postgres_utils::to_db_column_type(value, self.as_wasi_view().table())
            .map_err(Error::Other)?;
        let result = self
            .as_wasi_view()
            .table()
    ...
    async fn get(
        ...
        let value = self
            ...
            .value
            .clone();
        let result = postgres_utils::from_db_column_type(value, self.as_wasi_view().table())
            .map_err(Error::Other)?;
        Ok(result)
    }
...
impl<Ctx: WorkerCtx> HostLazyDbValue for DurableWorkerCtx<Ctx> {
    async fn create(&mut self, value: DbValue) -> anyhow::Result<Resource<LazyDbValueEntry>> {
        ...
        let value = postgres_utils::to_db_value(value, self.as_wasi_view().table())
            .map_err(Error::Other)?;
        let result = self
            .as_wasi_view()
            .table()
    ...
    async fn get(&mut self, self_: Resource<LazyDbValueEntry>) -> anyhow::Result<DbValue> {
        ...
        let value = self
            ...
            .value
            .clone();
        let result = postgres_utils::from_db_value(value, self.as_wasi_view().table())
            .map_err(Error::Other)?;
        Ok(result)
    }
...
pub(crate) mod postgres_utils {
    use crate::durable_host::rdbms::postgres::{LazyDbColumnTypeEntry, LazyDbValueEntry};
    use crate::preview2::wasi::rdbms::postgres::{
        Composite, CompositeType, DbColumn, DbColumnType, DbRow, DbValue, Domain, DomainType,
        MacAddress, Uuid,
    };
    use crate::services::rdbms::postgres::types as postgres_types;
    ...
    pub(crate) fn from_db_value(
        ...
        match value {
            ...
            postgres_types::DbValue::Composite(v) => {
                let mut values: Vec<Resource<LazyDbValueEntry>> =
    ...
    pub(crate) fn from_db_column_type(
    ...
    pub mod tests {
        ...
        fn check_db_value(value: postgres_types::DbValue, resource_table: &mut ResourceTable) {
            let wit = postgres_utils::from_db_value(value.clone(), resource_table).unwrap();
            // println!("wit {:?}", wit);
            let value2 = postgres_utils::to_db_value(wit, resource_table).unwrap();
            check!(value == value2);
        }
        ...
        fn check_db_column_type(
            ...
            let wit = postgres_utils::from_db_column_type(value.clone(), resource_table).unwrap();

            // println!("wit {:?}", wit);
            let value2 = postgres_utils::to_db_column_type(wit, resource_table).unwrap();
